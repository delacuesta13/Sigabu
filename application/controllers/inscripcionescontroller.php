...
class InscripcionesController extends VanillaController {
	...
	/*
	 * listar las inscripciones a cursos de una persona
	 */
	function listar_inscripciones_persona ($dni = null) {
		
		## se recibe dni de la persona, y éste coincide con el patrón
		if (isset($dni) && preg_match('/^[\d]{5,20}$/', $dni)) {
			...
			if (count($data_persona)!=0) {
				...
				$campos_tabla = array(
					...
					## realizar los joins entre las tablas
					'join' => array(
						0 => 'inscripcion.persona_dni = \'' . $dni . '\'',
						1 => 'inscripcion.curso_id = curso.id',
						2 => 'curso.periodo_id = periodo.id',
						3 => 'periodo.id = perfil.periodo_id',
						4 => 'perfil.persona_dni = \'' . $dni . '\'',
						5 => 'perfil.perfil_multientidad = multientidad.id',
						6 => 'curso.actividad_id = actividad.id'
					)
				);
				 ...
				 * agrego las columnas al query
				 */
				$str_tablas_sql = 'FROM cursos curso, perfiles perfil, '; ## tablas de la consulta y sus aliases
				foreach ($campos_tabla as $tabla => $def) {
					## $tabla es una tabla
					if(array_key_exists('table', $def) && $def['table']) {
						$str_tablas_sql .= $tabla . ' ' . $def['alias'] . ', ';
	...
	function eliminar ($id_curso = null) {
		...
		if ($_SESSION['nivel'] >= $GLOBALS['menu_project'][strtolower($this->_controller)]['actions'][strtolower($this->_action)]['nivel']) {
			...
			/*
			 * NOTA: no se recibe el id del curso, porque se
			 * va a eliminar inscripciones desde la vista ver
			 * personas.
			 */
