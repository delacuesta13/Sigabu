...
$(function() {
	...
	$( "#regpag_inscripciones" ).change(function() {
		load_dataTable_insc ('inscripciones', <?php echo $dni?>, 1, $( "#regpag_inscripciones" ).val(), '<?php echo $sort?>', '<?php echo $order?>', $( "#search_inscripciones" ).val());
				...
				if (array_key_exists('table', $def) && $def['table']) {
					foreach ($def['fields'] as $field => $attr) {
						...
						echo (($attr['sort']) ? ((strtolower($def['alias'] . '.' . $field) == strtolower($sort)) ? ((strtolower($order) == 'asc') ?
								('<a onclick="load_dataTable_insc(\'inscripciones\', ' . $dni . ', 1, ' . $record . ', \'' . strtolower($def['alias'] . '.' . $field) . '\', \'desc\', \'\');" style="cursor:pointer;">' . $attr['text'] . $html->includeImg('asc.png', 'Ascendente') . '</a>')	:
								('<a onclick="load_dataTable_insc(\'inscripciones\', ' . $dni . ', 1, ' . $record . ', \'' . strtolower($def['alias'] . '.' . $field) . '\', \'asc\', \'\');" style="cursor:pointer;">' . $attr['text'] . $html->includeImg('desc.png', 'Descendente') . '</a>')) :
								('<a onclick="load_dataTable_insc(\'inscripciones\', ' . $dni . ', 1, ' . $record . ', \'' . strtolower($def['alias'] . '.' . $field) . '\', \'asc\', \'\');" style="cursor:pointer;">' . $attr['text'] . '</a>')) :
						($attr['text']));
						?>
						</th>
						<?php
		...
		<div class="pagination">
			...
			for ($i = $pag_inicio; $i <= $pag_ultima; $i++) {
				...
				if ($i==$pag_inicio && $pagina==1) {
					...
					echo '<a rel="prev" class="prev_page" onclick="load_dataTable_insc(\'inscripciones\', ' . $dni . ', ' . ($pagina - 1) . ', ' . $record . ', \'' . $sort . '\', \'' . $order . '\', \'' . $q . '\');"
				...
				if ($i==$pagina) {
					...
					echo '<a onclick="load_dataTable_insc(\'inscripciones\', ' . $dni . ', ' . $i . ', ' . $record . ', \'' . $sort . '\', \'' . $order . '\', \'' . $q . '\');"
				...
				if ($i==$pag_ultima && $pagina==$numpag) {
					...
					echo '<a rel="next" class="next_page" onclick="load_dataTable_insc(\'inscripciones\', ' . $dni . ', ' . ($pagina + 1) . ', ' . $record . ', \'' . $sort . '\', \'' . $order . '\', \'' . $q . '\');"
